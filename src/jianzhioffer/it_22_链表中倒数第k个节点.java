package jianzhioffer;

import BaseUtil.ListUtil;
import it_Base.IListNode;

import java.util.ArrayList;

/**
 * 输入一个链表，输出该链表中倒数第k个节点。为了符合大多数人的习惯，本题从1开始计数，即链表的尾节点是倒数第1个节点。
 *
 * 例如，一个链表有 6 个节点，从头节点开始，它们的值依次是 1、2、3、4、5、6。这个链表的倒数第 3 个节点是值为 4 的节点。
 *
 */
public class it_22_链表中倒数第k个节点 {
    public static void main(String[] args) {
        int[] nums={1,2,3,4,5};
        IListNode head=IListNode.makeNode(nums);
        int k=2;
        IListNode res=new it_22_链表中倒数第k个节点().getKthFromEnd(head,k);
        ArrayList<Integer> traverse = IListNode.traverse(res);
        ListUtil.arrayOne(traverse);
    }

    public IListNode getKthFromEnd(IListNode head, int k) {
        IListNode pre=new IListNode(0);
        pre.next=head;
        IListNode fast=pre,slow=pre;
        while (k>0){
            fast=fast.next;
            k--;
        }
        while (fast!=null){
            fast=fast.next;
            slow=slow.next;
        }
        return slow;

    }
}
